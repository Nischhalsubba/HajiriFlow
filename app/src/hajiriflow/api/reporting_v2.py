...
from hajiriflow.reporting.evidence import merged_attendance_events
...
def _coverage_view(row: dict[str, object]) -> DepartmentCoverageRow:
    counts = defaultdict(int, row["status_counts"])
    return DepartmentCoverageRow(
        ...
        status_counts=dict(counts),
        employee_ids=list(row["employee_ids"]),
...
def department_coverage(
    ...
    try:
        ...
        return [_coverage_view(row) for row in rows]
    except (LookupError, ValueError) as exc:
        raise _bad_request(exc) from exc
...
def _export_table(
    *,
    ...
    if report_name == "monthly":
        ...
        metadata = [("From", from_date), ("To", to_date)]
        return "Monthly workforce summary", headers, values, metadata, False
