(() => {
  ...
  function captureFilterFocus(event) {
  ...
  function restoreFilterFocus() {
    if (!pendingFilterFocus || !workspace) return;
    const snapshot = pendingFilterFocus;
    pendingFilterFocus = null;
    const candidates = [...workspace.querySelectorAll("[data-filter]")];
    const target = candidates.find((candidate) => (
      candidate.dataset.filter === snapshot.filter
  ...
  document.addEventListener("input", captureFilterFocus);
  document.addEventListener("change", captureFilterFocus);
  ...
  if (workspace) {
    const observer = new MutationObserver(() => {
      enhance(workspace);
      if (pendingFilterFocus) requestAnimationFrame(restoreFilterFocus);
    });
