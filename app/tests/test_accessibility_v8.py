...
def test_filter_focus_is_restored_after_workspace_rerender() -> None:
    assert "pendingFilterFocus" in JS
    assert 'target.matches("[data-filter]")' in JS
    assert "restoreFilterFocus" in JS
    assert 'workspace.querySelectorAll("[data-filter]")' in JS
